  ...
  ══════════════════════════════════════════ -->
    <aside class="w-52 flex-shrink-0 bg-sidebar flex flex-col py-6 px-4 gap-5">
        ...
        <nav class="flex flex-col gap-1">
            <!-- Active -->
            <a href="/dashboard/{{ strtolower(Auth::user()->division->name ?? 'mkwk') }}"
        ...
        <div class="flex-1 overflow-y-auto px-8 pb-10 flex flex-col gap-5">
            ...
            <div>
                ...
                <p class="text-[10px] font-semibold tracking-[.2em] text-gray-400 mt-0.5 uppercase">
                    Welcome to the MKWK Division Dashboard
                </p>
            </div>
            ...
            <div class="grid grid-cols-3 gap-4">
                <div class="relative bg-white border border-black/10 rounded-3xl p-6 overflow-hidden shadow-sm">
                    ...
                    <p class="text-xs text-gray-500 mt-3 leading-relaxed">Total production boards currently in progress
                        within this division.</p>
                ...
                <div class="bg-[#F5B8B8] rounded-3xl p-6 shadow-sm">
                    ...
                    <p class="text-xs text-red font-semibold mt-3">↗ {{ $doneTasks }} of {{ $totalTasks }} tasks
                        completed.</p>
                </div>
                <div class="bg-[#E4DFD7] border border-black/10 rounded-3xl p-6 shadow-sm">
                    <p class="text-[10px] font-bold tracking-[.16em] uppercase text-gray-400 mb-2">Completed Projects
                    </p>
                    <p class="text-6xl font-black leading-none">{{ $completedProjects }}</p>
                    <p class="text-xs text-gray-500 mt-3 leading-relaxed">Projects where all tasks are fully resolved.
                    </p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-3 gap-4">
                <div class="col-span-2 bg-white border border-black/10 rounded-3xl p-6 shadow-sm">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <span
                            class="bg-red text-white text-[10px] font-bold px-3 py-1 rounded-full tracking-wide">Action
                            Required</span>
                    </div>
                        ...
                        @forelse($urgentTasks as $task)
                            <div class="flex items-start justify-between py-3">
                                <div>
                                    ...
                                    <p class="text-xs text-gray-400 mt-0.5">Proyek : {{ $task->project->name ?? '-' }}
                                    </p>
                                </div>
                    ...
                    @forelse($teamPerformance as $member)
                        <div class="{{ !$loop->last ? 'mb-5' : '' }}">
                            <div class="flex items-center justify-between mb-1.5">
                                ...
                                <span
                                    class="bg-red text-white text-[9px] font-bold px-2 py-0.5 rounded-full tracking-wide">{{ $member->completed_tasks }}
                                    TASKS</span>
                            </div>
                            <div class="h-1.5 bg-[#D3CEC6] rounded-full overflow-hidden">
                                <div class="h-full bg-red rounded-full"
                        ...
                        <div class="flex items-start gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-500 leading-relaxed mt-0.5">
                                    {{ Str::limit($activity->message, 120) }}</p>
